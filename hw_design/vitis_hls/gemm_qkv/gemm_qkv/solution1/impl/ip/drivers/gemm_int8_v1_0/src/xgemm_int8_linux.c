...
typedef struct {
    ...
    XGemm_int8_uio_map maps[ MAX_UIO_MAPS ];
} XGemm_int8_uio_info;

/***************** Variable Definitions **************************************/
static XGemm_int8_uio_info uio_info;
...
static int uio_info_read_name(XGemm_int8_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGemm_int8_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGemm_int8_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGemm_int8_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGemm_int8_Initialize(XGemm_int8 *InstancePtr, const char* InstanceName) {
	XGemm_int8_uio_info *InfoPtr = &uio_info;
